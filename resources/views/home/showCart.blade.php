...
<li><a href="index.php-route=checkout|cart.html">Shopping Cart</a></li>
</ul>
<div class="row">
...
<thead style="background-color:rgb(0,99,0)">
<tr class="cart_menu">
<th class="image" style="color:white;font-size:15px;text-align: center">Item</th>
<th class="description" style="color:white;font-size:15px;text-align: center">Name</th>
<th class="price" style="color:white;font-size:15px;text-align: center">Price</th>
<th class="quantity" style="color:white;font-size:15px;text-align: center">Quantity</th>
<th class="total" style="color:white;font-size:15px;text-align: center">Total</th>
<td></td>
</tr>
</thead>
<tbody>
...
<td class="cart_product" align="center">
	<a href=""><img src="{{URL::to('storage/app/public/'.$content->options->image)}}" width="80px"
		height="80px"></a>
</td>
<td class="cart_description" align="center">
	<h4 style="margin-top:20px"><a href="{{URL::to('productDetails/'.$content->id)}}">{{$content->name}}</a></h4>
</td>
<td class="cart_price" align="center">
	<p style="margin-top:20px">{{$content->price}}L.E</p>
</td>
<td class="cart_quantity" align="center">
	<div class="cart_quantity_button" style="margin-top:16px">
		<form class="form-horizontal" action="{{url('updateCart')}}" method="post" enctype="multipart/form-data">
              {{ csrf_field() }}
		<input class="cart_quantity_input" type="text" name="quantity" value="{{$content->qty}}" autocomplete="off" size="2">
		<input type="hidden" name="rowId" value="{{$content->rowId}}" />
		<button type="submit" value="update" class="btn btn-sm btn-success" >update</button>
	</form>
	</div>
</td>
<td class="cart_total" align="center">
	<p style="margin-top:20px" class="cart_total_price">{{$content->total}}</p>
</td>
...
<ul>
<li><h4>Cart Sub Total:<span class="text-success">{{Cart::subtotal()}}</span></h4></li>
<li><h4>Eco Tax :<span class="text-success">{{Cart::tax()}}</span></h4></li>
<li><h4>Shipping Cost :<span class="text-success">Free</span></h4></li>
<li><h4>Total :<span class="text-success">{{Cart::total()}}</span></h4></li>
<li>
@if($userId)
<form class="form-horizontal" action="{{url('makeOrder')}}" method="post" enctype="multipart/form-data">
              {{ csrf_field() }}
		<input class="form-control" type="text" name="location" placeholder="Enter your location" style="margin-top:10px" required>
		<input type="hidden" name="userId" value="{{$userId}}" />
		
	
@endif	
<a class="btn btn-default update" href="{{URL::to('destroyCart')}}">
<div>    
<button type="button" class="btn btn-default check_out">Delete Cart</button>
</div>
</a>
@if($userId)
